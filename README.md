...
| **`ui-ux`** | Designs landing pages and app screens that hold attention: live product UI instead of illustration, scroll-driven reversible motion, neutral surfaces with one accent. One divergent taste round, then a review loop where every complaint becomes the smallest fix plus a measurement. Ships the approved motion patterns and a Playwright suite that catches void scroll, blurred rest states, scenes that stop looping, content that needs JS, clipped text, text that fails contrast against its ground and abrupt folds. |
...
`confluence-docs`, `jira-tickets`, `social-carousel` and `quality-gate` drive a Go CLI, so installing them also puts a binary on your PATH. `business-ai-first` and `landing-seo-geo` are Markdown only. `ui-ux` ships Node scripts that run with the Playwright of the project under review.
