#!/bin/sh
# install.sh — installs the jira-tickets skill.
#
...
#
#   curl -fsSL https://raw.githubusercontent.com/diegoclair/harness/main/skills/jira-tickets/install/install.sh | sh
#
...
if command -v curl >/dev/null 2>&1; then
  curl -fsSL "$ROOT_URL" | sh -s -- install jira-tickets
elif command -v wget >/dev/null 2>&1; then
  wget -qO- "$ROOT_URL" | sh -s -- install jira-tickets
else
