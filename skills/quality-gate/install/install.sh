#!/bin/sh
# install.sh — installs the quality-gate skill.
#
# Kept at this URL for the published one-liner and for `quality-gate update`,
# which shells out to it. The work is done by the `skills` installer binary;
# this only forwards the skill name to it.
#
#   curl -fsSL https://raw.githubusercontent.com/diegoclair/harness/main/skills/quality-gate/install/install.sh | sh
#
...
if command -v curl >/dev/null 2>&1; then
  curl -fsSL "$ROOT_URL" | sh -s -- install quality-gate
elif command -v wget >/dev/null 2>&1; then
  wget -qO- "$ROOT_URL" | sh -s -- install quality-gate
else
