...
const defaultRepo = "diegoclair/skills"
...
func install(s Skill, opts installOptions) error {
	out := opts.Out
	plat, err := platform()
	if err != nil {
		return err
...
func resolveVersion(s Skill, opts installOptions) (string, error) {
	if opts.Version != "" {
...
// files were written.
func installPayload(s Skill, extractDir, dir, binDir string) (int, error) {
...
func verify(s Skill, binPath, version, dir string, out io.Writer) error {
...
// never printed twice.
func reportUnconfigured(s Skill, binPath string, checkErr error, out io.Writer) {
	var reason string
